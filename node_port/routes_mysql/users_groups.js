...
router.get('/all', authGuard, (req, res)=>{
    try{
        if(req.view_permissions.module_groups != undefined && req.view_permissions.module_groups){ 
...
router.post('/group_data', authGuard, (req, res)=>{
    try{
        if(req.view_permissions.module_groups != undefined && req.view_permissions.module_groups){ 
            if(req.body.id){
                db.query("SELECT * FROM admin_groups WHERE admin_group_id = '"+req.body.id+"' LIMIT 1", (err, rows, fields)=>{
                    if (err) throw err
                    res.send({
                        status: true,
                        user_data: rows[0]
        ...
        if(req.view_permissions.module_groups != undefined && req.view_permissions.module_groups){ 
            if(req.body.id){
                db.query("SELECT * FROM admin_groups WHERE admin_group_id = '"+req.body.id+"' LIMIT 1", (err, rows, fields)=>{
                    if (err) throw err
                    let update_obj = [];
                    if(req.body.name != undefined){ update_obj.push("`group_name` = '"+req.body.name+"'"); }
                    if(req.body.color != undefined){ update_obj.push("`group_color` = '"+req.body.color+"'"); }
                    if(update_obj.length > 0){
                        let update_str = "UPDATE admin_groups SET "+update_obj.join(" , ")+" WHERE admin_group_id = "+req.body.id;
