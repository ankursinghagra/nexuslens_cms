...
router.get("/all", authGuard, (req, res) => {
  try {
    ...
    if (
      req.view_permissions.module_users != undefined &&
      req.view_permissions.module_users
    ) {
      db.query(
        "SELECT * FROM admin_users AS U JOIN admin_groups AS G ON G.admin_group_id=U.admin_group ",
        (err, rows, fields) => {
          if (err) throw err;
          res.send(rows);
...
router.get("/systeminfo", async (req, res) => {
  try {
    db.query(
      "SELECT * FROM `admin_groups` ORDER BY `admin_groups`.`admin_group_id` ASC",
      (err, rows, fields) => {
        if (err) {
          return res.status(401).send("bad request");
        }
...
router.post("/userinfo", authGuard, async (req, res) => {
  try {
    ...
    db.query(
      "SELECT admin_email,admin_email_verified,admin_group,admin_id,admin_name,admin_photo FROM admin_users WHERE admin_email = '" +
        email +
        "' LIMIT 1 ",
      (err, rows, fields) => {
        if (err) throw err;
...
router.post("/user_data", authGuard, async (req, res) => {
  try {
    ...
    if (
      ...
      if (req.body.id) {
        db.query(
          "SELECT admin_id,admin_email,admin_name,admin_group FROM admin_users WHERE admin_id = '" +
            req.body.id +
            "' LIMIT 1 ",
          (err, rows, fields) => {
            if (err) throw err;
...
router.put("/user_data", authGuard, async (req, res) => {
  try {
    //console.log(req.body);
    if (
      req.modify_permissions.module_users != undefined &&
      req.modify_permissions.module_users
    ) {
      if (req.body.id) {
        db.query(
          "SELECT admin_id,admin_email,admin_name,admin_group FROM admin_users WHERE admin_id = '" +
          ...
          (err, rows, fields) => {
            ...
            if (req.body.name != undefined) {
              update_obj.push("`admin_name` = '" + req.body.name + "'");
            }
            if (req.body.email != undefined) {
              update_obj.push("`admin_email` = '" + req.body.email + "'");
            }
            if (req.body.group_id != undefined) {
              update_obj.push("`admin_group` = '" + req.body.group_id + "'");
            }
